...
#corners were considered
    ...
    def checkOuterCollision(self, char):
        #solve to find the corners of the character (C)
        CTL = (char.x, char.y)
        CBL = (char.x , char.y + char.length)
        CBR = (char.x + char.width, char.y + char.length)
        CTR = (char.x + char.width, char.y)
        char.cornersC = [CTL,CBL,CBR,CTR]

        #solve to find the 4 corners of the abstract (A) (wall, pit etc)
        ATL = (self.x, self.y)
        ABL = (self.x , self.y + self.length)
        ABR = (self.x + self.width, self.y + self.length)
        ATR = (self.x + self.width, self.y)
        self.cornersA = [ATL,ABL,ABR,ATR]
        #solve for the distance between each corner
        for elemC in char.cornersC:
            for elemA in self.cornersA:
    ...
    def move(self,char):
        ...
        if (self.checkOuterCollision(char)):
            if(self.num[1] < self.colDist or self.num[14] < self.colDist):
                self.y -= 1
            elif(self.num[3] < self.colDist or self.num[6] < self.colDist):
                self.x -= 1
            elif(self.num[4] < self.colDist or self.num[11] < self.colDist):
                self.y += 1
            elif(self.num[9] < self.colDist or self.num[12] < self.colDist):
                self.x += 1
            elif(self.num[2] < self.colDist):
                self.y -= 1
                self.x -= 1
            elif(self.num[7] < self.colDist):
                self.y += 1
                self.x -= 1
            elif(self.num[8] < self.colDist):
                self.y += 1
                self.x += 1
            elif(self.num[13] < self.colDist):
                self.y -= 1
                self.x += 1
